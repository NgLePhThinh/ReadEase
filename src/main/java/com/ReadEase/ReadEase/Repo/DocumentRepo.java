...
public interface DocumentRepo extends JpaRepository<Document, Long> {
    ...
    @Query(value = "select d.ID, d.name,  d.totalPages, d.numberOfPagesReading, d.star, d.createAt,d.lastRead, d.url, d.thumbnailLink " +
            "from user u inner join document d " +
            "where u.ID = d.USER_ID and u.ID = ?4 and d.name like %?3% " +
            "order by ?1 ?2 " +
            "limit ?6 " +
            "offset ?5", nativeQuery = true)
    List<Object[]> findDocumentCustom(String sortBy, String sortOrder, String name, String userID, int skip, int size);
    ...
    @Query(value = "select d.ID, d.name,  d.totalPages, d.numberOfPagesReading, d.star,  DATE_FORMAT(d.createAt, '%H:%i %d/%m/%Y') as createAtFormat ,DATE_FORMAT(d.lastRead, '%H:%i %d/%m/%Y') as lastReadFormat, d.url, d.thumbnailLink " +
            " from user u inner join document d " +
            "where u.ID = d.USER_ID and u.ID = ?1  " +
            "order by ?4 ?5 " +
            "limit ?3 " +
    ...
    @Query(value = "select d.ID, d.name,  d.totalPages, d.numberOfPagesReading, d.star,  DATE_FORMAT(d.createAt, '%H:%i %d/%m/%Y') as createAtFormat ,DATE_FORMAT(d.lastRead, '%H:%i %d/%m/%Y') as lastReadFormat, d.url, d.thumbnailLink  " +
            ...
            "where u.ID = d.USER_ID and u.ID = ?1 and d.ID = cd.DOCUMENT_ID and c.ID = COLLECTION_ID and c.ID = ?6 " +
            "order by ?4 ?5 " +
            "limit ?3 " +
            "offset ?2", nativeQuery = true)
    List<Object[]> findDocumentCustomByColID(String userID, int i, int size, String sortBy, String sortOrder, int colID);
}
